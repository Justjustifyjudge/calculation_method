...
df = pd.read_excel(r'C:\Users\linyiwu\Desktop\计算方法实验\lin_-\实验二\concrete+compressive+strength\Concrete_Data.xls', header=0)
...
# 保存图表为图片
plt.savefig(r'lin_-\实验二\photos\con_table_and_chart.png')
plt.clf()
# 创建并保存表格图片
table_data = {
    'Lambda (Alpha)': alphas,
...
plt.axis('off')  # 隐藏坐标轴
plt.savefig(r'lin_-\实验二\photos\con_2_table.png', bbox_inches='tight')
